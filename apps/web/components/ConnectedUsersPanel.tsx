// apps/web/components/ConnectedUsersPanel.tsx
"use client";

import { useState, useEffect } from "react";
import { Users, Wifi, WifiOff, X, Search, MessageCircle } from "lucide-react";
...
export default function ConnectedUsersPanel({ workspaceId }: ConnectedUsersPanelProps) {
  ...
  useEffect(() => {
    ...
    fetchUsers();
    const interval = setInterval(fetchUsers, 30000); // Polling cada 30s
...
// Modal de chat integrado (simple y funcional)
function DirectMessageModal({
  ...
  useEffect(() => {
    fetchMessages();
    const interval = setInterval(fetchMessages, 5000); // Polling cada 5s
    return () => clearInterval(interval);
  }, [user.id]);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[300px]">
          {messages.length === 0 ? (
            ...
            messages.map((msg) => {
              const isMine = msg.senderId === user.id ? false : true;
              return (
                <div
                  key={msg.id}
