// apps/web/components/TaskMembersSection.tsx
"use client";
import { useState, useEffect } from "react";
import { UserPlus, Users, Clock, Check, Trash2, UserCircle } from "lucide-react";

interface TaskMembersSectionProps {
  taskId: string;
  workspaceId: string;
  currentUserId: string;
  currentAssigneeId: string | null;
  onAssigneeChanged: (newId: string | null) => void;
}
...
export function TaskMembersSection({
  ...
  // ✅ CORREGIDO: Cargar miembros de la tarea Y del workspace al montar o cambiar taskId/workspaceId
  useEffect(() => {
    fetchMembers();
    if (workspaceId) {
      ...
      <div>
        ...
        {assignee ? (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <Check className="absolute -bottom-1 -right-1 w-4 h-4 text-cyan-400 bg-slate-900 rounded-full" />
                </div>
                <div>
              ...
              <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] font-medium ${getElapsedTimeColor(assignee.timeAgo)}`}>
          ...
          <div className="p-4 bg-slate-950/40 border border-slate-800 rounded-xl text-center">
            <p className="text-xs text-slate-500">No hay responsable asignado</p>
            <p className="text-[10px] text-slate-600 mt-1">Usa el selector de arriba para asignar uno</p>
          </div>
        )}
      </div>
